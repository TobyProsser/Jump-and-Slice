...
public class ContentPlayerController : MonoBehaviour
{
    ...
    ContentMovementController playerMovementController;
    ...
    void Start()
    {
        playerMovementController = this.GetComponent<ContentMovementController>();
        ...
        StartCoroutine(CheckIfEnemiesOnTile());
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.transform.tag == "Tile" && collision.gameObject != curTile)
        {
            ...
            if (CheckTileForEnemies(curTile)) camControl.enemiesOnTile = true;
            ...
            {
                AudioManager.instance.Play("NormalLand");
            }

            StartCoroutine(TurnOffCine());
        }
        ...
        if (collision.transform.tag == "Bullet")
        {
            ...
            if (PlayerSwordController.curSwordSpeed < 150 && PlayerSwordController.curSwordSpeed > -150)
            {
                //if not dead and shield isnt active take damage
                if (!hasDied && !shieldActive)
                {
                    health -= 1;
                    AudioManager.instance.Play("LoseHeart");
                }
                UpdateHealth();
                if (health <= 0 && !hasDied)
                {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            AudioManager.instance.Play("PotionBreak");
